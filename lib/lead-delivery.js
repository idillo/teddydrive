...
function runtimeBuyers() {
  const raw = process.env.BUYER_ROUTING_CONFIG_V2;
  if (!raw) throw new Error('BUYER_ROUTING_CONFIG_V2 is not configured in this deployment');
  let buyers;
  try { buyers = JSON.parse(raw); }
  ...
  const active = buyers.filter(buyer => buyer && buyer.delivery_mode !== 'off').sort((a, b) => (a.priority || 100) - (b.priority || 100));
  if (!active.length) throw new Error('BUYER_ROUTING_CONFIG_V2 contains no active buyers');
  return active;
}
...
async function routeLead(lead, buyers) {
  if (!buyers) buyers = runtimeBuyers();
...
module.exports = { prepareLead, insertLead, saveLead, runtimeBuyers, routeLead, persistDelivery, persistRoutingFailure, processLead, deliverToBuyer, redact, payloadFor };
