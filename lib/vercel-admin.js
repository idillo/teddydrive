...
async function upsertEnvironmentVariables(items) {
  ...
  for (const item of items) {
    const payload = { key: item.key, value: item.value, type: item.type || 'sensitive', target: ['production', 'preview'], comment: 'Managed by TeddyDrive Admin' };
...
async function verifyEnvironmentVariable(key, expectedValue) {
  const project = encodeURIComponent(required('VERCEL_PROJECT_ID'));
  const listed = await vercel(`/v10/projects/${project}/env${query()}`);
  const variables = Array.isArray(listed) ? listed : listed.envs || [];
  const variable = variables.find(item => item.key === key && item.target?.includes('production'));
  if (!variable) throw new Error(`Vercel did not save ${key} for Production`);
  const saved = await vercel(`/v10/projects/${project}/env/${encodeURIComponent(variable.id)}${query()}`);
  if (saved.value !== expectedValue) throw new Error(`Vercel verification failed for ${key}; deployment was not started`);
}
async function saveBuyerToken(id, token) {
  const key = tokenKey(id);
  await upsertEnvironmentVariables([{ key, value: token }]);
  await verifyEnvironmentVariable(key, token);
  return { key, lastFour: token.slice(-4) };
}
async function buyerTokenStatus(buyers) {
  const project = encodeURIComponent(required('VERCEL_PROJECT_ID'));
  const listed = await vercel(`/v10/projects/${project}/env${query()}`);
  const variables = Array.isArray(listed) ? listed : listed.envs || [];
  const productionKeys = new Set(variables.filter(item => item.target?.includes('production')).map(item => item.key));
  return buyers.map(buyer => ({ ...buyer, token_configured: Boolean(buyer.auth_env_var && productionKeys.has(buyer.auth_env_var)) }));
}
...
async function publishRouting(buyers) {
  ...
  if (!safe.length) throw new Error('Cannot publish routing without an active buyer');
  const snapshot = JSON.stringify(safe);
  await upsertEnvironmentVariables([{ key: 'BUYER_ROUTING_CONFIG_V2', value: snapshot, type: 'encrypted' }]);
  await verifyEnvironmentVariable('BUYER_ROUTING_CONFIG_V2', snapshot);
...
module.exports = { tokenKey, routingBuyer, saveBuyerToken, buyerTokenStatus, publishRouting };
